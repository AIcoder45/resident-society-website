...
export function GalleryCard({ item, className, compact = false }: GalleryCardProps) {
  ...
  // Debug logging in development - only on client side
  React.useEffect(() => {
    if (typeof window !== "undefined" && process.env.NODE_ENV === "development" && (item.youtubeUrl || item.instagramUrl)) {
      console.log(`GalleryCard for "${item.title}":`, {
        youtubeUrl: item.youtubeUrl,
        instagramUrl: item.instagramUrl,
        hasYoutubeUrl: !!item.youtubeUrl,
        hasInstagramUrl: !!item.instagramUrl,
      });
    }
  }, [item.title, item.youtubeUrl, item.instagramUrl]);
    ...
    <>
      ...
      >
        <div className="relative h-full w-full">
          <Link href="/gallery" className="absolute inset-0 z-0 touch-manipulation tap-feedback" aria-label={`View gallery: ${item.title}`} />
          {cardContent}
        </div>
      </motion.div>
      ...
      {selectedGalleryItem && (
        <ImageCarousel
          ...
          youtubeUrl={selectedGalleryItem.item.youtubeUrl}
          instagramUrl={selectedGalleryItem.item.instagramUrl}
        />
      )}
    </>
  );
